...
module.exports.renderMypage = (req, res) => {
    res.redirect('/mypage/mypost-post');
};

module.exports.renderMyPostPage = async(req, res) => {
...
module.exports.renderMyCommentPage = async(req, res) => {
    const { id, role } = req.user;
    const { page } = req.query;
...
module.exports.renderMyLikePostPage = async(req, res) => {
    const { id, role } = req.user;
    const { page } = req.query;
...
module.exports.renderMyLikeCommentPage = async(req, res) => {
    const { id, role } = req.user;
    const { page } = req.query;
...
module.exports.renderMyReportPostPage = async(req, res) => {
    const { id, role } = req.user;
    const { page } = req.query;
...
module.exports.renderMyReportCommentPage = async(req, res) => {
    const { id, role } = req.user;
    const { page } = req.query;
...
module.exports.renderMyNoteReceivedPage = async(req, res) => { // 받은 쪽지 페이지
    const {id, role} = req.user;
    const { page } = req.query;
...
module.exports.renderMyNoteSentPage = async(req, res) => { // 보낸 쪽지 페이지
    const {id, role} = req.user;
    const { page } = req.query;
...
module.exports.renderMyNoteInboxPage = async(req, res) => { // 쪽지 보관함 페이지
...
module.exports.renderNoteForm = async(req, res) => {   // 쪽지 쓰기
    res.render('mypage/sendNote')
};

module.exports.sendNote = async(req, res) => {  // 쪽지 보내기
...
module.exports.viewNote = async(req, res) => {   // 쪽지 보기
...
module.exports.deleteNote = async(req, res) => {  // 쪽지 삭제.
    for(noteId of req.body) {
        const note = await Note.findById(noteId).populate('sender').populate('recipient');
...
module.exports.renderMyNotiPage = async(req, res) => {  // 알림 페이지
    const {id, role} = req.user;
...
module.exports.loadMyNotiMore = async(req, res) => { // 알림 페이지 무한스크롤 
    const {id} = req.user;
    const {skip} = req.body;
...
module.exports.checkMyNotiUnread = async(req, res) => {   // 안읽은 알림이 있는지 체크
    const {id} = req.user;
    const notis = await Notification.findOne({recipient:id, isRead:false});
    res.json(notis);
};

module.exports.checkMyNoti = async(req, res) => {   // 알림 페이지 및 nav알림 체크
...
module.exports.checkAllMyNoti = async(req, res) => {   // 모든 알림 읽음 표시
    const {id} = req.user;
    await Notification.updateMany({recipient:id}, {$set:{isRead:true}});
    res.status(200).json();
};

module.exports.loadNavNoti = async(req, res) => {    // navbar알림
    const {id} = req.user;
